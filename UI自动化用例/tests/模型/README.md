...
设 `PW_HEADLESS=1` 或 `CI` 环境变量时自动无头。
...
- 代码保留中文注释，说明用到的 Playwright 技术与断言目标
- 统一使用 web-first 断言与接口等待，不使用固定 `sleep`
- 断言失败时，按「定位器 / 等待时序 / 测试数据 / 环境 / 产品缺陷」五类分别定位
