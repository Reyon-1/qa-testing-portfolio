...
param(
    ...
    [ValidateSet('T0','P1','P2','P3','P4','P5','S1','S2','S3','T3')]
    [string]$StartAt = 'T0',
...
$Root = 'C:\Users\ZhuanZ1\Desktop\压测报告\GLM 5.2 压测'
...
try {
    ...
    foreach ($stage in $Stages) {
        if ($stage.Name -eq $StartAt) { $startReached = $true }
        if (-not $startReached) { continue }
